...
# from models.model_h2 import CNNPolicy, MLPPolicy
from models.model_h3 import CNNPolicy, MLPPolicy
from agents.gail_discriminator import GailAgent
...
#             self.actor_critic = MLPPolicy(obs_shape[0], action_space, H1=256, H2=128, H3=64, obs_norm=0)
            self.actor_critic = MLPPolicy(obs_shape[0], action_space, H1=128, H2=128, H3=128, obs_norm=1)
#             self.actor_critic = MLPPolicy(obs_shape[0], action_space, H1=64, H2=64, obs_norm=0)
...
#         print('obs_shape', obs_shape)
        ...
        # gail begin
        self.use_gail = conf['use_gail']
        if self.use_gail:
            self.gail = GailAgent(obs_shape[0], action_shape, MODEL_ID=MODEL_ID, tsbx=self.tsbx)
            self.gail.load_expert(conf['gail_expert'])
        # gail end
...
#         self.rollouts.states[0].copy_(self.current_state)
        ...
        # gail begin
        self.episode_rewards_gail = torch.zeros([conf['num_processes'], 1])
        self.final_rewards_gail = torch.zeros([conf['num_processes'], 1])
        # gail end
    ...
    def load_expert(self, name):
        self.gail.load_expert(name)
    
    def get_final_rewards(self, gail=0):
        if gail:
            return self.final_rewards_gail
    ...
    def on_store(self, step, state, reward, done):
        ...
        if self.use_gail:
            ...
            # gail reward
            reward_gail = self.gail.expert_reward(state, self.current_action.data.numpy()).float()
#             print('state', state.shape)
#             print('action', self.current_action.data.numpy().shape)
#             print('reward', reward.size())
            # reward finetune
#             reward_gail.pow_(0.9)   # good
            reward_gail.pow_(1.2)   # good
            # end finetune
            self.episode_rewards_gail += reward_gail
...
#         self.episode_rewards += reward
        ...
        # gail begin
        if self.use_gail:
            self.final_rewards_gail *= masks
            self.final_rewards_gail += (1 - masks) * self.episode_rewards_gail
            self.episode_rewards_gail *= masks
        # gail end
        if self.conf['cuda']:
            masks = masks.cuda()
    ...
    def optimize(self, j=0):
        ...
        # gail begin
#         if self.use_gail:
#             self.gail.optimize(rollouts.states[:-1].view(-1, *self.obs_shape), 
...
#                                                 self.conf['batch_size'] * self.conf['num_processes'], drop_last=False)
                # PPO DEBUG BEGIN
                sampler = BatchSampler(SubsetRandomSampler(range(self.conf['num_processes'] * self.conf['num_steps'])), 
                                                self.conf['batch_size'], drop_last=False)
                # PPO DEBUG END
                for indices in sampler:
                    ...
                    # gail for PPO begin
                    if self.use_gail:
                        log_step = j if gail_steps==j else 1
                        self.gail.optimize(states_batch, 
                                                actions_batch,
                                                n=1,
                                                log_step=log_step,
                                                )
