...
from models.model_h3_test import CNNPolicy, MLPPolicy
...
#         self.rollouts.states[0].copy_(self.current_state)
    ...
    def on_action(self, step, obs_new=None):
        # Multi Target Fix
        if obs_new is not None:
            self.rollouts.update(step, obs_new)
        # Multi Target End
        if self.train:
            value, action, log_prob, dist_entropy = self.actor_critic.act_eval(Variable(self.rollouts.states[step]))
            #~ value, action, log_prob, dist_entropy = self.actor_critic.act_eval(Variable(self.rollouts.states[step], volatile=True))
    ...
    def optimize(self, j=0):
        ...
        if self.conf['algo'] in ['a2c', 'acktr']:
            ...
            # debug begin
            #~ print('action_log_probs0', action_log_probs[0].data.numpy())
            # debug end
            ...
            # debug begin
            #~ print('action_log_probs', action_log_probs[0].data.numpy())
            #~ print('values', values[0])
            #~ print('values', values.size(), values.requires_grad)
            #~ print('action_log_probs', action_log_probs.size(), action_log_probs.requires_grad)
            #~ print('dist_entropy0', torch.cat(self.dist_entropy_batch).size())
            #~ print('dist_entropy', dist_entropy.size(), dist_entropy.requires_grad)
            #~ print('rollouts.returns[:-1]', rollouts.returns[:-1].shape)
            # debug end
            ...
            if self.conf['algo'] == 'a2c':
                ...
                if self.tsbx and j%100==0 and 1:
                    for name, param in actor_critic.named_parameters():
                        ...
                        #~ self.tsbx.add_histogram('actor_critic.%s.grads'%name, param.grad.clone().data.numpy(), j)
                # DEBUG END
...
#         model_name = "{}_{}_{}_{:.0f}".format(a2c_name, j, t_save, reward_save_max)
    ...
    def load_model(self, conf):
        ...
        if conf.get('trained_model', None):
            ...
            actor_critic = torch.load(os.path.join(load_dir, conf['trained_model'] + ".pt"))
            if conf['algo'] == 'acktr':
